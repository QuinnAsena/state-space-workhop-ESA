...
- organised into a site-by-species matrix $Y$, and a covariate matrix $X$ by that resolution

Now we can fit the model.

### Finding initial conditions using `mnGLMM()`

The `mnTS()` function will provide estimates for biotic interactions ($C$ matrix), taxa-driver relationships ($B$ matrix), and cross-correlated error ($V$ natrix). But the model needs initial starting values for these parameters to begin with. We get initial starting conditions from the data by running the `mnGLMM()` funcion. `mnGLMM` returns estimates for the $B$ and $V$ matrices (not the $C$ matrix) and assumes that there are no time gaps in the data.

::: {.callout-tip}

The arguments of the _starting_ values in both the `mnGLMM()` and `mnTS()` are all suffixed with `.start` (e.g., `B.start` will be the initial values for the $B$ matrix).
...
The model needs inputs for _both_ `.start` and `.fixed` for each matrix: $B$, $C$, and $V$.
...
We will fit the model twice: without taxa interactions, and without taxa interactions. Remember, interactions are handled in the $C$ matrix and we do `mnGLMM()` does not estimate $C$.

The starting values of the $B$, and $V$ paramters will be the same for both models (without, and with interactions), and they will be taken from the output of `mnGLMM()`.
