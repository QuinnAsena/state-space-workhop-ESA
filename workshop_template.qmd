...
We will use data have been already wrangled, and the focal taxa chosen. The whole process from the start to finish of downloading, wrangling, and fitting the data is coded in the state-space-walkthrough.qmd document. 
...
```{r}
story_pollen_matrix <- as.matrix(story_binned %>% select(-c(bins, age, char_acc))) # Select taxa
...
## Answer
```{r}
C.start.mnTS35 = 0.5 * diag(n) # creates an n x n matrix with diagonal elements
C.start.mnTS35[3, 5] = 0.5
C.fixed.mnTS35 <- C.start.mnTS35
C.fixed.mnTS35[C.fixed.mnTS35 != 0] <- NA
```
:::
...
I got a good fit by including an _Fagus-Ulmus_ interaction. But it took about 7 minutes to fit. 
```{r}
start_time <- Sys.time()
mnTS_mod35 <- mnTS(Y = story_pollen_matrix[sample_idx, ],
                 X = story_char_matrix_scaled, Tsample = sample_idx,
                 B0.start = B0.start.mnTS, B0.fixed = B0.fixed.mnTS,
                 B.start = B.start.mnTS, B.fixed = B.fixed.mnTS,
                 C.start = C.start.mnTS35, C.fixed = C.fixed.mnTS35,
                 V.start = V.start.mnTS, V.fixed = V.fixed.mnTS,
                 dispersion.fixed = 1, maxit.optim = 1e+06)
...
::: {.callout-tip}
## Question
1. Create new input matrices for `C.start`, and `C.fixed` that estimates an interaction between only two species.
...
data.frame(without_interactions = mnTS_mod_int$AIC,
           with_interactions = mnTS_mod$AIC,
           with_interaction35 = mnTS_mod35$AIC)
```
:::
